...
def gitBrake(anno_path):
    ...
    ds = np.loadtxt(anno_path, delimiter=",", dtype="float32")
...
def gitAnno(anno_path):
    ...
    ds = np.loadtxt(anno_path, delimiter=",", dtype="float32")
    shape_ds = ds.shape
    anno = np.zeros(shape=(shape_ds[0], shape_ds[1] - 3), dtype="float32")
    # anno[:, 0] = ds[:, 1]  # brake
    anno[:, 0] = ds[:, 2]  # ws
    anno[:, 1] = ds[:, 3]  # thot
    anno[:, 2] = ds[:, 5]  # acc
    ...
    return annotations, anno.shape, t_anno
...
def gitDS_MLP(anno_path, bs):
    ...
    brake_anno, norm_brake = gitBrake(anno_path=anno_path)
    (train_anno, dim_input, t_anno) = gitAnno(anno_path=anno_path)

    DATASET = tf.data.Dataset.zip((train_anno, brake_anno))

    # t_anno = t_anno.astype("float32")
    # norm_brake = norm_brake.astype("float32")
    # DATASET = tf.data.Dataset.from_tensor_slices((t_anno, norm_brake))
...
def gitDS(data_dir, BatchSize):
    ...
    # (anno, dim_input, t_anno) = gitAnno(anno_path)
    # anno = anno.batch(BatchSize)
    (annotations, normBrake) = gitBrake(anno_path=anno_path)
    ...
    # DATASET = tf.data.Dataset.zip((imgs, annotations))
